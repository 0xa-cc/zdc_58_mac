...
static php_stream_filter *php_mcrypt_filter_create(const char *filtername, zval *filterparams, int persistent TSRMLS_DC)
{
	...
	iv = emalloc(iv_len + 1);
	if (zend_hash_find(HASH_OF(filterparams), "iv", sizeof("iv"), (void**) &tmpzval) == SUCCESS) {
		if (Z_TYPE_PP(tmpzval) != IS_STRING) {
			php_error_docref(NULL TSRMLS_CC, E_WARNING, "Filter parameter[iv] must be of type: string");
			return NULL;
		}
