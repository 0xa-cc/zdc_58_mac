...
char * ExcerptGen_c::BuildExcerpt ( const ExcerptQuery_t & q, CSphDict * pDict, ISphTokenizer * pTokenizer )
{
	...
	m_sBuffer.SetBinary((const char*)pStartPtr, pTokenizer->GetBufferEnd() - pStartPtr);
	while ( ( sWord = pTokenizer->GetToken() ) != NULL )
	{
		const char * pTokenStart = pTokenizer->GetTokenStart ();

		if ( pTokenStart != pStartPtr )
			AddJunk ( pLastTokenEnd - pStartPtr, pTokenStart - pLastTokenEnd ); //when found a space, addjunk called.
		...
		pLastTokenEnd = pTokenizer->GetTokenEnd (); //return the current token's end

		m_dTokens.Resize ( m_dTokens.GetLength () + 1 ); //append a new token to m_dTokens.
		Token_t & tLast = m_dTokens.Last ();
	...
	m_dResult.Add ( 0 );
	// FIXME: in GBK mode, UTF-8 string always longer than GBK, so no needs to check the output buffer length again. 
	char * pRes = new char [ m_dResult.GetLength() +1];
	pTokenizer->GetLocalBuffer((BYTE*)&m_dResult[0], m_dResult.GetLength(),(BYTE*)pRes);
