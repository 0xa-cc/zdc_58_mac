...
CSphSource * SpawnSourcePython ( const CSphConfigSection & hSource, const char * sSourceName )
{
	...
	//should read the token file
	LOC_CHECK ( hSource, "python_script", "in source '%s'.", sSourceName );
	CSphSource * pSrcPy = NULL;

	CSphString sCommand = hSource["python_script"];

	CSphSource_Python * pXmlPipe = new CSphSource_Python ( sSourceName );
	if ( !pXmlPipe->Setup ( hSource, sCommand.cstr () ) )
		SafeDelete ( pXmlPipe );
...
int main ( int argc, char ** argv )
{
	...
	if ( !g_bQuiet )
		fprintf ( stdout, CORESEEK_BANNER );
