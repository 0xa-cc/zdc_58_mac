...
static KeyDesc_t g_dKeysSource[] =
{
	...
	{ "python_name",			0, NULL },
	{ "python_script",			0, NULL },
	{ "python_connection",			0, NULL },
	{ "class_path",			0, NULL },
	{ "sql_prefix",			0, NULL },
	{ "sql_type",			0, NULL },
...
#if USE_PYTHON
static KeyDesc_t g_dKeysExtension[] =
{
	{ "connection_string",		0, NULL },
	{ "python_layer",			0, NULL },	
	{ NULL,						0, NULL }
};
#endif
...
#if USE_PYTHON
	if ( !strcasecmp ( sKey, "extension" ) )		return true;
#endif
	return false;
}
...
#if USE_PYTHON
	else if ( m_sSectionType=="extension" )	pDesc = g_dKeysExtension;
#endif
	if ( !pDesc )
	{
...
#if USE_PYTHON
	/* reload python config bridge */
	if(m_tConf.Exists ( "extension") && m_tConf["extension"].Exists ( "extension" ) )
	{
		CSphConfigSection & tSec = m_tConf["extension"]["extension"];
		//printf("%s",tSec.GetStr("connection_string")); //get the connection string
		const char* db_conn = tSec.GetStr("connection_string");
		const char* py_layer = tSec.GetStr("python_layer");
		if (!Py_IsInitialized()) {
			Py_Initialize();
			if (!Py_IsInitialized()) {
				return false;
		...
		snprintf(buffer,sizeof(buffer),"g_csft_connection_string='%s'",db_conn);
		PyRun_SimpleString(buffer);
		Run_PySimpleFile(py_layer); //__pollute__ the global name space.
		//reload all index(s) and source(s)		
		InitExtendConf();
	}
#endif
	return true;
...
/////////////////////////////////////////////////////////////////////////////
#if USE_PYTHON
#define PY_CHECKFUNCTION_CALLABLE(x)	\
	{ if(!x||!PyCallable_Check(x)) { Py_XDECREF(x);	return 0; }	}
int		CSphConfigParser::InitExtendConf()
{
	...
	PyObject* pFunc_GetSourceList = PyObject_GetAttrString(main_module, "CSFT_GetSourceList");
	PyObject* pFunc_GetSourceConf = PyObject_GetAttrString(main_module, "CSFT_GetSourceConf");
	PyObject* pFunc_GetIndexList = PyObject_GetAttrString(main_module, "CSFT_GetIndexList");
	PyObject* pFunc_GetIndexConf = PyObject_GetAttrString(main_module, "CSFT_GetIndexConf");

	PY_CHECKFUNCTION_CALLABLE(pFunc_GetSourceList);
	PY_CHECKFUNCTION_CALLABLE(pFunc_GetSourceConf);
	PY_CHECKFUNCTION_CALLABLE(pFunc_GetIndexList);
	PY_CHECKFUNCTION_CALLABLE(pFunc_GetIndexConf);

	PyObject* pArgs  = Py_BuildValue("()");
	PyObject* pList = PyEval_CallObject(pFunc_GetSourceList, pArgs);   
	PyObject* pListIndex = PyEval_CallObject(pFunc_GetIndexList, pArgs);
	if (!PyList_Check(pList) || !PyList_Check(pListIndex)) {
		Py_XDECREF(pList);
		Py_XDECREF(pListIndex);
		Py_XDECREF(pArgs);
		Py_XDECREF(pFunc_GetSourceList);
		Py_XDECREF(pFunc_GetSourceConf);
		Py_XDECREF(pFunc_GetIndexList);
		Py_XDECREF(pFunc_GetIndexConf);
	}
	//source
	{
		size_t i = 0;
		...
		for(i = 0; i < size; i++) {
			...
			char* strval = PyString_AsString(item_str);
			PyObject* pArgsKey  = Py_BuildValue("(s)",strval);
			PyObject* pListAttr = PyEval_CallObject(pFunc_GetSourceConf, pArgsKey);   
			if(PyDict_Check(pListAttr)){
				nRet = DealExtendConfSection("source",strval, pListAttr);
			}
			Py_XDECREF(pListAttr);
			Py_XDECREF(pArgsKey);
			Py_XDECREF(item_str); 
		}
	}
	...
	{
		size_t i = 0;
		size_t size = PyList_Size(pListIndex);
		int nRet = 0;
		for(i = 0; i < size; i++) {
			//printf
			PyObject* item = PyList_GetItem(pListIndex,i);
			PyObject* item_str = PyObject_Str(item);
			char* strval = PyString_AsString(item_str);
			PyObject* pArgsKey  = Py_BuildValue("(s)",strval);
			PyObject* pListAttr = PyEval_CallObject(pFunc_GetIndexConf, pArgsKey);   
			if(PyDict_Check(pListAttr)){
				nRet = DealExtendConfSection("index",strval, pListAttr);
			}
			Py_XDECREF(pListAttr);
			Py_XDECREF(pArgsKey);
			Py_XDECREF(item_str); 
...
#undef PY_CHECKFUNCTION_CALLABLE

int		CSphConfigParser::DealExtendConfSection(const char * sType, const char* section, PyObject* pList)
{
	PyObject *key, *value;
	int pos = 0; 
	if ( !AddSection ( sType, section ) )
		return 0;
	int n = 0;
	while (PyDict_Next(pList, &pos, &key, &value)) {
		char* strkey = PyString_AsString(key);
		if(PyString_Check(value)) {
			char* strval = PyString_AsString(value);
			AddKey(strkey,strval);
		}
...
bool sphConfTokenizer ( const CSphConfigSection & hIndex, CSphTokenizerSettings & tSettings, CSphString & sError )
{
	...
	else if (hIndex("charset_dictpath") && hIndex["charset_type"]=="zh_cn.utf-8" )
	{
		...
		/*
		int nBext = 1;
		if (hIndex("charset_nbest"))
			nBext = hIndex["charset_nbest"].intval();
	
		pTokenizer = sphCreateUTF8ChineseTokenizer(
						hIndex["charset_dictpath"].cstr(),
						nBext);
		*/
		tSettings.m_nBest = 1;
		if (hIndex("charset_nbest"))
			tSettings.m_nBest = hIndex["charset_nbest"].intval();
